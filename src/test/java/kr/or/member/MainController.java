...
public class MainController {
	...
	public static void main(String[] args) throws IOException {
		...
		//크기가 3인 MemberVO클래스를 배열객체로 선언 m1,m2,m3
		...
		try {//예외가 발생할 수 있는 구문을 try안에 넣기
			timeClient.open();
			//throw new RuntimeException();
		} catch (SocketException e) {//try 구문에서 예외가 발생하면 catch 문으로 가서 해당 예외 처리
			System.out.println("open메서드 실행 오류");
			e.printStackTrace();
		} finally {//finlly는 try/catch 구문을 수행하고 '반드시'수행되는 구문
	    ...
	    System.out.println("로컬 PC 시간: " + LocalDateTime.now());//로컬시간
